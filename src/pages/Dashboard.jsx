...
import { Ticket, Clock, AlertTriangle, CheckCircle, TrendingUp, ArrowRight, User, Copy, Check, ExternalLink, MessageSquare } from 'lucide-react';
...
export default function Dashboard() {
  ...
  const handleCopyMessage = () => {
    navigator.clipboard.writeText(`Hi! You can submit a support ticket using the button below:\n\n${ticketUrl}`);
    setCopiedMsg(true);
    setTimeout(() => setCopiedMsg(false), 2000);
  };

  useEffect(() => {
    db.Ticket.list('-created_date', 100).then(data => {
      setTickets(data || []);
      setLoading(false);
    });
  }, []);
